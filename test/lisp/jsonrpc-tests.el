...
(ert-deftest jsonrpc-connection-send-contortion ()
  ...
  (cl-letf (((symbol-function 'initialize-instance) #'ignore)
    ...
    (should (equal (jsonrpc--stringify-method (list :method 'foo :id 100))
                   '(:method "foo" :id 100)))
    (should (equal (jsonrpc--stringify-method (list :id 100 :method :foo))
                   '(:id 100 :method "foo")))
    (should (equal (jsonrpc--stringify-method (list :method "foo" :id 100))
                   '(:method "foo" :id 100)))
    (should (equal (jsonrpc--stringify-method (list :method nil :id 100))
                   '(:id 100)))
    (should (equal (jsonrpc--stringify-method (list :id 100 :method '(foo)))
                   '(:id 100)))
    (should (equal (jsonrpc--stringify-method (list :id 100 :method))
                   '(:id 100 :method)))))
...
(ert-deftest jsonrpc-process-connection-sham-genericity ()
  ...
  (let* ((name "sham-genericity")
    ...
    (let ((conn (make-instance 'jsonrpc-process-connection
      ...
      (should (buffer-live-p too-bad))
      (should (cl-some (lambda (x) (cl-search name x))
                       (mapcar #'buffer-name (buffer-list))))
      (jsonrpc-shutdown conn t)
      (let (kill-buffer-query-functions)
        (kill-buffer too-bad)
        (kill-buffer (jsonrpc-events-buffer conn)))
      (should-not (cl-some (lambda (x) (cl-search name x))
                           (mapcar #'buffer-name (buffer-list)))))))
