...
extra args."
  (when (and (symbolp (car form))
	...
	(byte-compile-warn
         "`%s' called with %d argument%s to fill %d format field%s"
         (car form)
         nargs (if (= nargs 1) "" "s")
  ...
  (let* ((this-kind (if macro-p
    ...
    (if (not (listp body))
      ...
      (let ((code (byte-compile-lambda (cons arglist body) t)))
        ...
        (byte-compile-flush-pending)
        (let ((newform `(defalias ',name
                          ,(if macro-p `'(macro . ,code) code) ,@rest)))
          (when byte-native-compiling
...
of the list FUN."
  ...
  (let* ((arglist (nth 1 fun))
	...
	(setq doc (help-add-fundoc-usage doc arglist)))
...
(defun byte-compile-defvar (form &optional toplevel)
  (let* ((fun (nth 0 form))
    ...
    (if toplevel
        ...
        (when (or (cddr form)       ; `value' provided.
		  (not (eq (car form) 'defvar))) ;not just a declaration.
