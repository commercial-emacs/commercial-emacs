...
make_lisp_ptr (void *xpntr0, enum Lisp_Type type)
{
  void *xpntr = (type == Lisp_Symbol)
    ...
    : xpntr0;
  Lisp_Object obj = TAG_PTR_INITIALLY (type, xpntr);
...
#define SX_ADMITS_COMPARISON(pvec_type)	\
  (SX_ADMITS_ANYTHING (pvec_type) && \
       (pvec_type <= PVEC_NORMAL_VECTOR || pvec_type >= PVEC_CLOSURE))
...
extern int emacs_spawn (pid_t *, int, int, int, Lisp_Object,
			char **, char **, const char *, const char *,
			bool, bool, const sigset_t *);
