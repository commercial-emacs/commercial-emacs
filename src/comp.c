  ...
  (Lisp_Object filename)
{
  ...
  const char *cfilename = SSDATA (ENCODE_FILE (Fexpand_file_name (filename, Qnil)));
  Lisp_Object package_user_dir = !NILP (Fboundp (intern ("package-user-dir")))
    ? Fexpand_file_name (Fsymbol_value (intern ("package-user-dir")), Qnil)
    : Qnil;
  if (!NILP (package_user_dir))
    fprintf (stderr, "got here %s %s\n", cfilename, SSDATA (package_user_dir));
  if (!NILP (package_user_dir)
      && strstr (cfilename, SSDATA (package_user_dir)))
    {
      /* rename dance to sync filesystem */
    ...
    comp_unit->handle = dynlib_open_for_eln (cfilename);
  if (!comp_unit->handle)
