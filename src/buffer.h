...
/* Blandy, Mcgrath, and Monnier conspired successfully to make an
   ...
   *conversion* semantics from forwarded variables to buffer local
   ...
   The confusion exploded when code paths for localized forwarded
   slots [2] diverged from other localized forwarded variables.

   [1] More specifically, one of the fields over which
   FOR_EACH_PER_BUFFER_OBJECT_AT iterates, (and excludes
   `undo_list_`).

   [2] And less conspicuously, kboard fields.
*/
...
extern Lisp_Object kill_local_variable_internal (struct Lisp_Symbol *sym,
...
extern ptrdiff_t previous_overlay_change (ptrdiff_t, Lisp_Object);
extern ptrdiff_t next_overlay_change (ptrdiff_t, Lisp_Object);
