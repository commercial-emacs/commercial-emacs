...
command_loop (void)
{
  ...
  for (;;)
      ...
      else if (keylen == -1)
	{
      ...
      if (XWINDOW (selected_window)->start_dirty) {
        struct buffer *b;
        XWINDOW (selected_window)->start_dirty = false;
		     ...
		     Time t)
{
  ...
  if (WINDOWP (window_or_frame))
      ...
      if (part == ON_TEXT)
	{
	  ...
	  yret = wy - WINDOW_BORDER_WIDTH (w) - WINDOW_TAB_LINE_HEIGHT (w) - WINDOW_HEADER_LINE_HEIGHT (w);
	}
      ...
      else if (part == ON_LEFT_FRINGE)
	{
	  ...
	  dy = yret = wy - WINDOW_BORDER_WIDTH (w) - WINDOW_TAB_LINE_HEIGHT (w) - WINDOW_HEADER_LINE_HEIGHT (w);
	}
