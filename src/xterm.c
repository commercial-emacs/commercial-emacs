...
/* INPUT
   ...
   The primary entrypoint for input is XTread_socket().

   When pselect() detects input from the X server, or when the X
   server signals, x_filter_event() first runs the event through X
   Input Method (XIM) or GTK, which could for instance map keystrokes
   to international characters unavailable to the keyboard.  Then
   handle_one_xevent() translates the filtered Xlib events into struct
   input_events, which queue to the keyboard buffer.
   ...
   correspond to a given frame.  The function x_any_window_to_frame()
   searches for a frame by top-level window and widget windows.  Other
   functions search by specific window types, such as
   x_top_window_to_frame (searches by top-level window), and
   x_menubar_window_to_frame (searches by frame menu bars).

   INPUT FOCUS

   A window receiving an explicit FocusIn event is said to receive
   "explicit focus."  In the absence of such a window, a window
   receives "implicit focus" whenever the pointer enters it.  A frame
   is said to assert input focus if it contains an explicitly focused
   window, or in the absence of such a window, contains an implicitly
   focused window.  The function x_detect_focus_change implements
   the classification.

   REDISPLAY

   Every frame contains a struct terminal.  A graphics-capable
   terminal contains a non-NULL struct redisplay_interface (RIF).

   Examples of RIF methods include x_clear_frame_area,
   x_draw_window_cursor, and importantly, x_draw_glyph_string, which
   renders a glyph sequence of the same type and face along with
   decorations such as box face, underline, and overline.

   GRAPHICS CONTEXTS

   Chapter 7 of the Xlib Programming Manual.

   A graphics context (GC) is an X server-side object containing
   drawing attributes like fill style, stipple, and foreground
   and background pixel values.

   A GC is computed for each initial rendering of a given face, then
   reused.  See prepare_face_for_display in xfaces.c.

   Different GCs are used for drawing glyph strings in the presence of
   the cursor or mouse sensitive text.  For example, the frame's
   cursor GC is used in the common case where the default font applies
   to the cursor and the default face applies to the text beneath.
   In all other cases, a temporary context adjusts cursor color so
   that the text underneath stays visible.

   COLOR ALLOCATION

   Chapter 6 of the Xlib Programming Manual.

   Color allocation occurs in x_alloc_nearest_color_1.  Accommodation
   for the TrueColor visual class occurs in x_make_truecolor_pixel.
   Also see x_query_colors for mapping pixel values to color values.

   Emacs allocates color cells when a face is realized or when a frame
   changes its foreground and background colors.  As well, color cells
   are then released alongside the face or frame.  See calls to
   unload_color and load_color in xterm.c, xfaces.c and xfns.c.

   Color allocation is more self-contained under other window systems.
   Emacs simply passes 24 (or 32-bit) RGB values.
   ...
   derivatives such as neXTaw and Xaw3d) for dialog boxes and,
   optionally, scroll bars.  It does not support file panels.  The
   latter uses either Motif or LessTif for menu bars, popup menus,
   dialogs and file panels.

   A frame under the no toolkit configuration associates with only one
   window.

   Under Xt, every widget is its own X window.  The "outer widget" of
   class ApplicationShell, is the top-level window of a frame accessed
   via FRAME_OUTER_WINDOW().  A child "edit widget" of class
   EmacsFrame, controls the size of a frame and sustains draw
   operations.

   Under GTK, widgets do not necessarily correspond to X windows,
   since the toolkit might keep only a client-side record of certain
   widgets for performance reasons.  Because the GtkFixed widget
   holding the edit area might not correspond to an X window, drawing
   operations may be directly performed on the outer window, with
   special care taken to not overwrite the surrounding GTK widgets.
   Outer windows under GTK, accessed via FRAME_X_WINDOW, thus take on
   greater importance.

   Under no toolkit or Xt, X events are identified via bespoke calls
   to XPending and XNextEvent, then passed to handle_one_xevent.

   Under GTK, handle_one_xevent is installed as an event filter
   in GTK's event loop.

   handle_one_xevent accepts a FINISH callback parameter which
   forwards the event to XtDispatchEvent or GTK, under Xt or GTK
   respectively.

   FRAME RESIZING

   The toolkit and window manager express frame size via
   FRAME_PIXEL_WIDTH and FRAME_PIXEL_HEIGHT. The width and height of
   frame *Lisp* objects are expressed in "text size".  Use
   FRAME_TEXT_TO_PIXEL_WIDTH, FRAME_TEXT_TO_PIXEL_HEIGHT,
   FRAME_PIXEL_TO_TEXT_WIDTH and FRAME_PIXEL_TO_TEXT_HEIGHT to
   translate between the two domains.

   Any frame resize, whether explicit (`set-frame-size') or implicit
   (toggling scroll bars), triggers adjust_frame_size which determines
   if set_window_size_hook may run, and if so, dispatches to
   x_set_window_size().

   If the frame is visible, Emacs must wait for a ConfigureNotify
   event whereupon resize_frame_windows() gets called.  Given the
   costliness of redisplaying buffers, Emacs may collapse several
   ConfigureNotify events into one, such as during a mouse drag frame
   resize.

   ConfigureEvent events specifying the new width and height trigger
   change_frame_size() which decides whether a resize request can
   proceed immediately.  Since resizing windows can run arbitrary Lisp
   code, Emacs cannot generally process resize requests during
   redisplay (see xdisp.c commentary), and stores the new dimensions
   in the new_width and new_height slots of the frame.

   The redisplay code calls do_pending_window_change() at judicious moments
   to consummate the delayed size changes via resize_frame_windows.

   In addition to explicitly passing width and height values into
   gtk_window_resize or XResizeWindow, Emacs can also set window
   manager size hints.  See x_wm_set_size_hint in gtkutil.c.

   Consult the variable `frame-size-history' for assistance debugging
   frame resizing.

   HAVE_XSYNC

   The window manager or X server may believe a window resized before
   Emacs can process the ConfigureNotify event.

   Emacs announces support for the X Synchronization Extension by
   ...
   Emacs then constructs and saves a counter value from the provided
   low and high 32 bits.  Then, when redisplay processes a
   ConfigureNotify event, we set the counter to the saved value,
   indicating to the compositing manager that the window contents may
   be safely displayed.  The window manager may also postpone updating
   window decorations until this moment.
   ...
   For asynchronous requests that don't require a reply from the X
   server, call x_ignore_errors_for_next_request to begin ignoring
   errors, then call x_stop_ignoring_errors to reinstate them.

   For requests requiring error details, or otherwise require
   roundtrips to the X server, use the following functions:
