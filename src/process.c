...
DEFUN ("set-process-filter", Fset_process_filter, Sset_process_filter,
       2, 2, 0,
       doc: /* Give PROCESS the filter function FILTER.
A null FILTER selects the default filter.
A FILTER of t halts output processing.
Otherwise the provided FILTER must take two arguments:
the process and the current string output.  */)
  ...
  if (p->infd >= 0)
    {
      ...
      /* If p->filter WAS t (and network proc not stopped), then
	 resume reading output.  */
  ...
  (ptrdiff_t nargs, Lisp_Object *args)
{
  ...
  if (CONSP (tem))
    ...
    {
      XPROCESS (proc)->pty_in
	= XPROCESS (proc)->pty_out
  ...
  USE_SAFE_ALLOCA;
  const Lisp_Object input_sys = plist_get (contact, QCcoding);
  Lisp_Object decode_sys = CONSP (input_sys)
    ? XCAR (input_sys)
    : !NILP (input_sys)
    ? input_sys
    : Vcoding_system_for_read;
  Lisp_Object encode_sys = CONSP (input_sys)
    ? XCDR (input_sys)
    : !NILP (input_sys)
    ? input_sys
    : Vcoding_system_for_write;
  if (NILP (decode_sys) || NILP (encode_sys))
    {
      if (!NILP (program))
	{
	  ...
	  args2[i++] = name;
	  args2[i++] = buffer;
	  for (Lisp_Object tail = command; CONSP (tail); tail = XCDR (tail))
	    args2[i++] = XCAR (tail);
	  Lisp_Object found = Ffind_operation_coding_system (nargs2, args2);
	  if (CONSP (found))
	    {
	      if (NILP (decode_sys))
		decode_sys = XCAR (found);
	      if (NILP (encode_sys))
		encode_sys = XCDR (found);
	    }
	}
      if (CONSP (Vdefault_process_coding_system))
	{
	  if (NILP (decode_sys))
	    decode_sys = XCAR (Vdefault_process_coding_system);
	  if (NILP (encode_sys))
	    encode_sys = XCDR (Vdefault_process_coding_system);
	}
    }

  pset_decode_coding_system (XPROCESS (proc), decode_sys);
  pset_encode_coding_system (XPROCESS (proc), encode_sys);
   ...
   Lisp_Object just_this_one)
{
  ...
  return 0 < wait_reading_process_output (secs, nsecs, 0, 0,
					  NILP (process) ? NULL : XPROCESS (process),
					  NILP (just_this_one) ? 0 : (FIXNUMP (just_this_one) ? -1 : 1))
    ? Qt : Qnil;
}
...
#define HOST_FORMAT_IN6 "%x:%x:%x:%x:%x:%x:%x:%x"
  ...
  if (EQ (ps->filter, Qinternal_default_process_filter)
      || EQ (ps->filter, Qt))
    {
      /* The trivial filters dump to a buffer named after the process
	 buffer or, failing that, the process name.  */
      buffer = NILP (ps->buffer) ? ps->name : Fbuffer_name (ps->buffer);
      if (!NILP (buffer))
	{
	  args[1] = buffer;
...
error_handler_common (void)
{
  ...
  process_error_pause_time = 0; /* avoid infinite recursion */
  if (restore > 0)
    Fsleep_for (make_fixnum (restore), Qnil);
